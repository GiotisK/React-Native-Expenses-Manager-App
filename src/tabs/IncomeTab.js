...
class IncomeTab extends React.Component {
  ...
  fillArrays(param,value,totalIncome,totalOutcome){
    ...
    for (const day of dayKeys){//iterates all days of a month

      if(!isNaN(day)){ //if key is a number, and not 'dayincome' etc..
        ...
        for(var i in value[param][day]['data']){
          ...
          if(row.category ==='' && row.amount !=''){
            if(row.description == ''){
              incomeArr.push({description: 'No description', amount: row.amount})
            }else{
              incomeArr.push({description: row.description, amount: row.amount})
            }
            ...
            if(row.description == ''){
              outcomeArr.push({description: 'No description', category: row.category, amount: row.amount})
            }else{
              outcomeArr.push({description: row.description, category: row.category, amount: row.amount})
            } 
    ...
    return (
      <View>
        <MonthYearHeader
          ...
          monthOnValueChange = {(itemValue, itemIndex)=>{month=itemValue; this.getRowsFromAsyncStorage()}}
          yearSelectedValue = {year}
          yearOnValueChange ={(itemValue, itemIndex)=>{year=itemValue; this.getRowsFromAsyncStorage()}}
        />
        <View style={styles.container}>
            ...
            <View style={{ 
                ...
                >
                    <Text style={{margin:2, fontSize: leftTextSize, color:'white'}} >   Income   </Text>
                </TouchableOpacity>
            </View>
          ...
          <View style={{ 
                ...
                >
                    <Text style={{ margin:2, fontSize: rightTextSize, color:'white'}} > Outcome </Text>
                </TouchableOpacity>
          </View>
